...
#then use the email to send message
        email = EmailMessage(
            subject= f"{name} from doctor family.",
            body=message,
            from_email=settings.EMAIL_HOST_USER,   #sending email to myself in this case because user acnnot give credentials :)
...
class ManageAppointmentTemplateView(TemplateView):
    ...
    login_required = True
    paginate_by =3 #allows only 3 in the page
    ...
    def get_context_data(self,*args, **kwargs):
        ...
        appointments = Appointment.objects.all() #get appointments from the model/database
        context.update({   
            "appointments": appointments,
            "title":"Manage Appointments"  #we can see the title of the page
        })
        return context
